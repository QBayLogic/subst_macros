...
/// Make a list of substitutions in the input and make a callback with the result
///
/// The caller is responsible for guarding this with a `const _: () = { ... }` if necessary to avoid
/// polluting the module namespace or overwriting an existing macro def. This macro will a
/// definition of `run_substitution` for each substitution given.
///
...
macro_rules! serial_subst {
    ...
    (
        @run
        dollar: [$dollar:tt],
        substitutions: [[sub [$($lhs:tt)+] = [$($rhs:tt)*]] $($rest:tt)*],
        callback: $callback:tt,
        $($tokens:tt)*
    ) => {
